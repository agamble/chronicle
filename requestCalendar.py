...
def importICStoDatabase(url, d, user):
	r = requests.get(url)
	...
	for groups in course_ids:
		if not d.query('SELECT * FROM groups WHERE name=?', [groups[1]]):
			...
			mygroupid = d.query("SELECT * FROM groups WHERE name=?", [groups[1]])[0][0]
		ucl_to_us_dict[groups[0]] = mygroupid
	print ucl_to_us_dict
	print ucl_to_us_dict.values()

	for value in ucl_to_us_dict.values():
		user_group_tuple = (user[2], int(value))

		enrolment[id] = d.write_to_db('INSERT INTO enrolment (groupid, userid) VALUES (?,?)', user_group_tuple)
